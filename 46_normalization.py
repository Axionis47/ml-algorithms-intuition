...
def create_classification_task(n_samples: int = 1000, input_dim: int = 32,
                                n_classes: int = 4) -> Tuple:
    """
    Create a multi-class classification task.

    XOR-style patterns extended to multiple classes.
    """
    np.random.seed(42)
    X = np.random.randn(n_samples, input_dim)

    # Create class labels based on quadrant in first two dimensions
    ...
    return X[:split], y_onehot[:split], X[split:], y_onehot[split:], y[:split], y[split:]
...
class SimpleNetWithNorm:
    """
    Simple MLP with optional normalization for training experiments.
    """

    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int,
                 n_layers: int = 5, norm_type: str = 'none'):
        """
        Args:
        ...
        for i in range(n_layers):
            ...
            if i < n_layers - 1:  # No norm on output layer
                if norm_type == 'batch':
                    self.norms.append(BatchNorm1D(dims[i+1]))
    ...
    def forward(self, x: np.ndarray, training: bool = True) -> Tuple[np.ndarray, List]:
        ...
        cache = {'activations': [x], 'pre_norms': [], 'pre_activations': []}
        h = x

        for i in range(self.n_layers - 1):
            z = h @ self.weights[i] + self.biases[i]
            cache['pre_norms'].append(z)
        ...
        # Output layer (no norm, no activation - softmax in loss)
        logits = h @ self.weights[-1] + self.biases[-1]
        output = softmax_stable(logits)
        cache['logits'] = logits

        return output, cache

    def get_activation_stats(self, x: np.ndarray) -> List[Tuple[float, float]]:
        """Get mean and std of activations at each layer."""
        stats = []
        h = x
        ...
        for i in range(self.n_layers - 1):
            ...
            if self.norms[i] is not None:
                z = self.norms[i].forward(z)
            h = np.maximum(0, z)
            stats.append((np.mean(np.abs(h)), np.std(h)))

        return stats


def train_classifier(model: SimpleNetWithNorm, X_train: np.ndarray,
                     y_train: np.ndarray, epochs: int = 100,
                     lr: float = 0.01, batch_size: int = 32) -> dict:
    """
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, n_samples, batch_size):
            ...
            # Forward
            pred, cache = model.forward(batch_X, training=True)
            loss = cross_entropy_loss(pred, batch_y)
            epoch_loss += loss * (end - start)

            # Backward (simplified)
            grad = (pred - batch_y) / (end - start)

            # Update output layer
            h = cache['activations'][-1]
            model.weights[-1] -= lr * (h.T @ grad)
            model.biases[-1] -= lr * np.sum(grad, axis=0, keepdims=True)

            # Backprop through hidden layers
            dh = grad @ model.weights[-1].T
            for i in range(model.n_layers - 2, -1, -1):
                # ReLU derivative
                dz = dh * (cache['pre_activations'][i] > 0)

                # Update weights
                h_prev = cache['activations'][i]
                model.weights[i] -= lr * (h_prev.T @ dz)
                model.biases[i] -= lr * np.sum(dz, axis=0, keepdims=True)

                if i > 0:
                    dh = dz @ model.weights[i].T

        results['losses'].append(epoch_loss / n_samples)

        # Compute accuracy
        pred, _ = model.forward(X_train, training=False)
        acc = np.mean(np.argmax(pred, axis=1) == np.argmax(y_train, axis=1))
        results['accuracies'].append(acc)

        # Track activation statistics every 10 epochs
        if epoch % 10 == 0:
            stats = model.get_activation_stats(X_train[:100])
            results['activation_stats'].append(stats)
...
def experiment_training_stability(n_layers: int = 8,
                                   hidden_dim: int = 64,
                                   epochs: int = 100) -> dict:
    """
    THE KEY EXPERIMENT: Train networks with different normalizations.

    WHAT TO OBSERVE:
    - No normalization: Training unstable, may fail
    - Batch Norm: Stable training
    - Layer Norm: Stable training, works with any batch size
    """
    print("=" * 60)
    print("EXPERIMENT: Training Stability with Normalization")
    print("=" * 60)
    print("\nTraining networks with NO norm, BatchNorm, and LayerNorm...\n")

    # Create task
    X_train, y_train, X_test, y_test, _, _ = create_classification_task(
        n_samples=800, input_dim=32, n_classes=4
    )
    ...
    for norm_type in ['none', 'batch', 'layer']:
        ...
        model = SimpleNetWithNorm(
            input_dim=32, hidden_dim=hidden_dim, output_dim=4,
            n_layers=n_layers, norm_type=norm_type
        )

        train_results = train_classifier(
            model, X_train, y_train, epochs=epochs, lr=0.05, batch_size=64
        )

        results[norm_type] = train_results

        # Test accuracy
        pred, _ = model.forward(X_test, training=False)
        test_acc = np.mean(np.argmax(pred, axis=1) == np.argmax(y_test, axis=1))
        results[norm_type]['test_accuracy'] = test_acc

        print(f"  Final train loss: {train_results['losses'][-1]:.4f}")
        print(f"  Final train acc:  {train_results['accuracies'][-1]:.4f}")
        print(f"  Test accuracy:    {test_acc:.4f}")

    return results


def experiment_batch_size_training(batch_sizes: List[int] = [4, 8, 16, 32, 64, 128],
                                    n_layers: int = 6,
                                    epochs: int = 80) -> dict:
    """
    Show how batch size affects training with different norms.

    WHAT TO OBSERVE:
    - Batch Norm: Unstable with small batches (noisy statistics)
    - Layer Norm: Consistent across all batch sizes
    """
    print("\n" + "=" * 60)
    print("EXPERIMENT: Batch Size Effect on Training")
    print("=" * 60)
    ...
    results = {'batch_sizes': batch_sizes, 'batch_norm': [], 'layer_norm': []}

    for batch_size in batch_sizes:
        print(f"\nBatch size = {batch_size}")

        # Train with BatchNorm
        bn_model = SimpleNetWithNorm(32, 64, 4, n_layers, 'batch')
        bn_results = train_classifier(bn_model, X_train, y_train,
                                       epochs=epochs, lr=0.05, batch_size=batch_size)
        pred, _ = bn_model.forward(X_test, training=False)
        bn_acc = np.mean(np.argmax(pred, axis=1) == np.argmax(y_test, axis=1))

        # Train with LayerNorm
        ln_model = SimpleNetWithNorm(32, 64, 4, n_layers, 'layer')
        ln_results = train_classifier(ln_model, X_train, y_train,
                                       epochs=epochs, lr=0.05, batch_size=batch_size)
        pred, _ = ln_model.forward(X_test, training=False)
        ln_acc = np.mean(np.argmax(pred, axis=1) == np.argmax(y_test, axis=1))

        results['batch_norm'].append({
            'final_loss': bn_results['losses'][-1],
            'test_acc': bn_acc,
            'loss_variance': np.var(bn_results['losses'][-20:])
        })
        results['layer_norm'].append({
            'final_loss': ln_results['losses'][-1],
            'test_acc': ln_acc,
            'loss_variance': np.var(ln_results['losses'][-20:])
        })

        print(f"  BatchNorm: acc={bn_acc:.3f}, loss_var={results['batch_norm'][-1]['loss_variance']:.4f}")
        print(f"  LayerNorm: acc={ln_acc:.3f}, loss_var={results['layer_norm'][-1]['loss_variance']:.4f}")
...
def visualize_training_stability(results: dict, save_path: Optional[str] = None):
    """
    Visualize training stability across normalization methods.
    ...
    # 1. Loss curves
    ax = axes[0, 0]
    colors = {'none': 'red', 'batch': 'blue', 'layer': 'green'}
    labels = {'none': 'No Normalization', 'batch': 'Batch Norm', 'layer': 'Layer Norm'}

    for norm_type in ['none', 'batch', 'layer']:
        losses = results[norm_type]['losses']
        ax.semilogy(losses, color=colors[norm_type], label=labels[norm_type], linewidth=2)

    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss (log scale)')
    ax.set_title('Training Loss: Normalization Enables Stable Training', fontweight='bold')
    ax.legend()
    ...
    ax = axes[0, 1]
    for norm_type in ['none', 'batch', 'layer']:
        accs = results[norm_type]['accuracies']
        ax.plot(accs, color=colors[norm_type], label=labels[norm_type], linewidth=2)

    ax.set_xlabel('Epoch')
    ax.set_ylabel('Training Accuracy')
    ax.set_title('Accuracy: With Normalization = Faster Convergence', fontweight='bold')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    test_accs = [results[n]['test_accuracy'] for n in norm_types]
    x = np.arange(3)
    bars = ax.bar(x, test_accs, color=[colors[n] for n in norm_types], alpha=0.7)
    ax.set_xticks(x)
    ax.set_xticklabels(['No Norm', 'BatchNorm', 'LayerNorm'])
    ax.set_ylabel('Test Accuracy')
    ax.set_title('Final Test Accuracy', fontweight='bold')
    ax.set_ylim(0, 1)

    for bar, acc in zip(bars, test_accs):
        ax.annotate(f'{acc:.2%}', (bar.get_x() + bar.get_width()/2, acc + 0.02),
    ...
    1. WITHOUT NORMALIZATION
       ...
       • Activations may explode or vanish
       • Requires careful initialization & small LR

    2. WITH BATCH NORMALIZATION
       • Stable training
       • Faster convergence
       • BUT: Depends on batch size

    3. WITH LAYER NORMALIZATION
       • Stable training
       • Works with ANY batch size
       • Preferred for Transformers/RNNs

    CONCLUSION:
    Normalization isn't just helpful — it's often
    NECESSARY for training deep networks reliably.
    """
    ax.text(0.05, 0.95, summary, transform=ax.transAxes, fontsize=11,
            verticalalignment='top', fontfamily='monospace',
            bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))

    plt.suptitle('Training Stability: The Power of Normalization',
...
def visualize_batch_size_effect_training(results: dict, save_path: Optional[str] = None):
    """
    Show how batch size affects BatchNorm vs LayerNorm during training.
    """
    ...
    ax = axes[0]
    bn_accs = [r['test_acc'] for r in results['batch_norm']]
    ln_accs = [r['test_acc'] for r in results['layer_norm']]

    ax.plot(batch_sizes, bn_accs, 'b-o', label='BatchNorm', linewidth=2, markersize=8)
    ax.plot(batch_sizes, ln_accs, 'g-s', label='LayerNorm', linewidth=2, markersize=8)

    ax.set_xlabel('Batch Size')
    ax.set_ylabel('Test Accuracy')
    ax.set_title('BatchNorm Struggles with Small Batches', fontweight='bold')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ax.set_xscale('log', base=2)

    # Highlight problematic region
    ax.axvspan(0, 16, alpha=0.2, color='red', label='Problematic for BN')

    # 2. Training loss variance
    ax = axes[1]
    bn_vars = [r['loss_variance'] for r in results['batch_norm']]
    ln_vars = [r['loss_variance'] for r in results['layer_norm']]

    ax.semilogy(batch_sizes, bn_vars, 'b-o', label='BatchNorm', linewidth=2, markersize=8)
    ax.semilogy(batch_sizes, ln_vars, 'g-s', label='LayerNorm', linewidth=2, markersize=8)

    ax.set_xlabel('Batch Size')
    ax.set_ylabel('Loss Variance (log scale)')
    ax.set_title('Training Stability: LayerNorm is Consistent', fontweight='bold')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ax.set_xscale('log', base=2)

    plt.suptitle('Batch Size Sensitivity: BatchNorm vs LayerNorm',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def experiment_failure_modes() -> dict:
    """
    WHAT BREAKS NORMALIZATION?

    This experiment explores edge cases and failure modes.
    """
    print("\n" + "=" * 60)
    print("EXPERIMENT: Normalization Failure Modes")
    print("=" * 60)

    results = {}

    # 1. EPSILON TOO SMALL
    print("\n1. EPSILON TOO SMALL (Division by Zero)")
    print("-" * 40)
    print("What if ε = 0 and variance = 0?")

    x_constant = np.ones((10, 32)) * 5.0  # Constant input → variance = 0

    for eps in [0, 1e-12, 1e-6, 1e-3]:
        try:
            # LayerNorm with different epsilon
            mean = x_constant.mean(axis=-1, keepdims=True)
            var = x_constant.var(axis=-1, keepdims=True)
            normalized = (x_constant - mean) / np.sqrt(var + eps)

            has_nan = np.any(np.isnan(normalized))
            has_inf = np.any(np.isinf(normalized))

            if has_nan or has_inf:
                print(f"  ε={eps}: NaN/Inf detected! ✗")
                results[f'eps_{eps}'] = 'NaN'
            else:
                print(f"  ε={eps}: OK (max value = {np.max(np.abs(normalized)):.2f}) ✓")
                results[f'eps_{eps}'] = 'OK'
        except Exception as e:
            print(f"  ε={eps}: Error - {e}")
            results[f'eps_{eps}'] = 'Error'

    print("\n  INSIGHT: ε > 0 is essential to prevent division by zero!")

    # 2. BATCHNORM WITH BATCH SIZE 1
    print("\n2. BATCHNORM WITH BATCH SIZE = 1")
    print("-" * 40)
    print("BatchNorm needs variance across batch. What if batch=1?")
    ...
    for batch_size in [1, 2, 4, 32]:
        ...
        if batch_size == 1:
            print(f"  Batch={batch_size}: Variance = 0 everywhere! (undefined)")
            results[f'batch_{batch_size}'] = 'Undefined'
        else:
            var_mean = var.mean()
            print(f"  Batch={batch_size}: Mean variance = {var_mean:.4f} ✓")
            results[f'batch_{batch_size}'] = var_mean

    print("\n  INSIGHT: BatchNorm fails with batch_size=1. Use LayerNorm or GroupNorm!")

    # 3. WRONG NORMALIZATION DIMENSION
    print("\n3. WRONG NORMALIZATION DIMENSION")
    print("-" * 40)
    print("Normalizing wrong dimension gives wrong results")

    x = np.random.randn(8, 32, 64)  # (batch, seq, features)

    # Correct: normalize features (axis=-1)
    correct = (x - x.mean(axis=-1, keepdims=True)) / (x.std(axis=-1, keepdims=True) + 1e-6)

    # Wrong: normalize batch (axis=0)
    wrong = (x - x.mean(axis=0, keepdims=True)) / (x.std(axis=0, keepdims=True) + 1e-6)

    # Check: After correct normalization, feature stats should be ~0, ~1
    correct_mean = np.abs(correct.mean(axis=-1)).mean()
    wrong_mean = np.abs(wrong.mean(axis=-1)).mean()

    print(f"  Correct (axis=-1): Feature mean ≈ {correct_mean:.6f}")
    print(f"  Wrong (axis=0):    Feature mean ≈ {wrong_mean:.6f}")
    results['correct_dim'] = correct_mean
    results['wrong_dim'] = wrong_mean

    print("\n  INSIGHT: Always verify which dimension you're normalizing!")

    return results


def visualize_failure_modes(results: dict, save_path: Optional[str] = None):
    """Visualize normalization failure modes."""
    fig, axes = plt.subplots(1, 3, figsize=(14, 4))

    # 1. Epsilon effect
    ax = axes[0]
    eps_vals = ['0', '1e-12', '1e-6', '1e-3']
    colors = ['red' if results.get(f'eps_{e}') == 'NaN' else 'green'
              for e in [0, 1e-12, 1e-6, 1e-3]]
    ax.bar(eps_vals, [1]*4, color=colors, alpha=0.7)
    ax.set_xlabel('Epsilon Value')
    ax.set_ylabel('Status (Green=OK, Red=NaN)')
    ax.set_title('Failure: ε Too Small', fontweight='bold')

    # 2. Batch size effect
    ax = axes[1]
    batch_sizes = [1, 2, 4, 32]
    variances = [0 if results.get(f'batch_{b}') == 'Undefined'
                 else results.get(f'batch_{b}', 0) for b in batch_sizes]
    colors = ['red' if v == 0 else 'green' for v in variances]
    ax.bar([str(b) for b in batch_sizes], variances, color=colors, alpha=0.7)
    ax.set_xlabel('Batch Size')
    ax.set_ylabel('Mean Variance')
    ax.set_title('Failure: BatchNorm with Small Batch', fontweight='bold')
    ...
    1. EPSILON TOO SMALL
       • ε = 0 causes division by zero
       • Use ε ≥ 1e-6

    2. BATCHNORM + BATCH SIZE = 1
       • Variance is undefined
       • Use LayerNorm or GroupNorm

    3. WRONG DIMENSION
       • BatchNorm: across batch
       • LayerNorm: across features
       • GroupNorm: across feature groups
    """
...
if __name__ == "__main__":
    ...
    # NEW: Key training stability experiment
    training_stability_results = experiment_training_stability()

    # NEW: Batch size effect on training
    batch_training_results = experiment_batch_size_training()

    # Experiment 1: Normalization effect on activations
    norm_results = experiment_normalization_effect()

    # Experiment 2: Batch size sensitivity (statistics)
    ...
    # NEW: Training stability visualization (THE KEY FIGURE)
    visualize_training_stability(training_stability_results, '46_normalization_training.png')

    # NEW: Batch size training comparison
    visualize_batch_size_effect_training(batch_training_results, '46_normalization_batch_training.png')
    ...
    visualize_failure_modes(failure_results, '46_norm_failures.png')
